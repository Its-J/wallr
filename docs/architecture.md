...
Animated GIF frames use a bounded 128 MiB cache. Small and ordinary 1080p
animations remain cached for smooth loop playback, while larger animations
fall back to bounded compressed/on-demand decoding instead of allowing one
wallpaper to consume hundreds of MiB.
...
When a previous shared-memory buffer is still in use, Wallr allocates another
slot and retains the Wayland buffer lifecycle until release. It never falls
back to a new GPU wallpaper texture merely because the compositor has not yet
released the previous slot; this keeps repeated static switches bounded and
prevents the resident-memory growth that a rapid benchmark can otherwise hide.
